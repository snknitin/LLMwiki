...
## Step 5 — Decide whether you actually want a second model copy

NVIDIA's official Spark example is:

```text
nvidia/nemotron-3-nano-omni
```

Downloading it in LM Studio creates an LM Studio copy even if the vLLM NVFP4 checkpoint already exists in `~/.cache/huggingface`. Choose one of these paths:

- If your immediate goal is tuned ODS/Hermes serving, finish the vLLM Nemotron tutorial and **skip this download**.
- If your immediate goal is loading the model from the LM Studio desktop UI, continue below and accept the duplicate disk usage.

Download the LM Studio package:
...
The download uses disk, not 128 GiB of resident model memory.
...
`--ttl 1800` asks LM Studio to unload it after 1,800 seconds, or 30 minutes, of inactivity.
...
Current LM Studio headless mode supports:

- just-in-time loading when a downloaded model is requested;
- a default idle time-to-live of about 60 minutes unless changed;
- automatic eviction of the previous JIT-loaded model when another needs its memory;
- a setting that keeps only the last JIT-loaded model.
...
ExecStart=/home/snknitin/.lmstudio/bin/lms server start --port 1234
