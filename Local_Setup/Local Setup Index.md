...
| SecondSpark single-node Qwen v0.30 lane | Installed but qualification-blocked 2026-09-25. The pinned `b8439110` MiaAI-Lab vLLM 0.30 lane passed raw API, streaming, smoke, tools, vision, four-stream concurrency, and a 100/100 tool loop. That first long-agent input measured 206K tokens, drove `MemAvailable` down to 3,208 MiB, and produced nine `NV_ERR_NO_MEMORY` events, so the memory/kernel gate failed. The server is stopped, SecondSpark recovered to about 118 GiB available, and no LiteLLM or Hermes route was added. FirstSpark keeps `spark-fast` and its LM Studio/Nemotron setup. See [[DGX Spark Single-Node Qwen 3.8 Flash Next v0.30 Tutorial]] and [[DGX Spark Frontier Model Qualification Results]]. |
