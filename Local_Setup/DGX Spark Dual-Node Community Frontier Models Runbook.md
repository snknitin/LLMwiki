...
|---|---:|---|---|
| Current `spark-fast` | First only | The restored 18 GiB KV/five-sequence profile uses about 48.2 GiB by `nvidia-smi`; the 2026-09-26 co-resident test left about 35 GiB `MemAvailable` with LM Studio Nemotron also loaded. SecondSpark remains independent. | May coexist with LM Studio Nemotron on FirstSpark when Qwen starts first, and with an **independent** model on SecondSpark. It cannot coexist with any recipe below because every dual-node recipe needs both nodes idle. |
...
| GLM 5.3 Flash EXL3 4 bpw | Both | The 850K DFlash baseline uses GMU 0.87 plus `INSTANTTENSOR_BUFFER_SIZE=536870912` (14.15 GiB KV). MTP/850K needs those settings plus `INSTANTTENSOR_MAX_FREE_MEM_USAGE=0.75` to load its largest tensor. DFlash/500K needs GMU 0.86, not the upstream 0.84: this pair measured 12.69 GiB available KV versus 10.98 GiB required and completed boot warm-up. Keep these as separate private profiles; do not transfer one profile's fix to another. | Exclusive two-node lane. Stop Spark LM Studio and sparkDash for first boot. Do not schedule downloads, indexing, or other large host jobs beside it. Use the GLM tutorial's Step 16 for MTP/500K A/B profiles and separate result records. |
